...
export class IonDigitKeyboard implements OnDestroy, OnInit {
    ...
    public static clickSub: Subject<any> = new Subject();
    static get onClick() { return this.clickSub; }
    // tslint:disable-next-line:member-ordering
    private static showSub: Subject<any> = new Subject();
    static get onShow() { return this.showSub; }
    // tslint:disable-next-line:member-ordering
    private static hideSub: Subject<any> = new Subject();
    ...
    @Input() set width(v: any) {
        const isPercent = String(v).indexOf('%') > -1 ? true : false;
    ...
    @Input() set leftActionOptions(v: boolean | ActionOptions) {
        ...
        if (typeof v === 'boolean') {
            ...
            // v === true && this.log('Left action button is set to "true", an empty button is displayed.');
        }
    }
     ...
     */
    public btnClick(event: any, key: any): void {
        ...
        key === 'left' && this.leftActionClick.emit();
        key === 'right' && this.rightActionClick.emit();
        typeof key === 'number' && this.numberClick.emit(key);
    }
     ...
     */
    // tslint:disable-next-line:member-ordering
    static destroy(callback: (success: boolean) => void = (success: boolean) => { }): void {
        if (this.component) {
