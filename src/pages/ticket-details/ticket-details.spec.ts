// import { async, ComponentFixture, inject, TestBed } from '@angular/core/testing';

// import { DecodeStringPipe } from '../../pipes/decode-string/decode-string';
// import { TicketItemView } from "../home/ticket-item-view/ticket-item-view";
// import { RecentActivityItem } from "../logs-page/recent-activity-item/recent-activity-item";
// // import { CardNotesPhotosView } from "./card-notes-photos-map-view/card-notes-photos-map-view";
// // import { RecentActivityExpanded } from "./card-notes-photos-map-view/recent-activity-expanded/recent-activity-expanded";
// import { TicketDetailsPage } from "./ticket-details";

// // import { TimeAgoPipe } from "../../pipes/hours-minutes-seconds";
// // import { KeysPipe } from "../../pipes/keys/keys";
// import { GlobalSearch } from "../global-search/global-search";

// import { VVSApp } from "../../providers/vvs-controller/vvs-controller";
// import { CONFIGURE_TESTBEST } from "../../providers/vvs-controller/vvs-controller.util.spec";


// jasmine.DEFAULT_TIMEOUT_INTERVAL = 20000;

// describe('TicketDetailsPage Component  >>  ', () => {

//     let fixture: ComponentFixture<TicketDetailsPage>;
//     let component: TicketDetailsPage;
//     // let de: DebugElement;
//     // let initializeData: Initialize;

//     // let vvsApp: VVSApp;

//     beforeEach(async(() => {

//         const declarations = [
//             TicketDetailsPage,
//             GlobalSearch,
//             // TimeAgoPipe,
//             // CardNotesPhotosView,
//             TicketItemView,
//             RecentActivityItem,
//             DecodeStringPipe,
//             // RecentActivityExpanded,
//             // KeysPipe
//         ];

//         CONFIGURE_TESTBEST(declarations);

//     }));

//     beforeEach( inject([VVSApp], ($vvsApp: VVSApp) => {

//         // $vvsApp.socketService.socketService.subscribe();
//         //
//         // $vvsApp.socketService
//         //        .initialize(init.init.LoginUser.Authorization,
//         //                    init.init.LoginUser.CurrentProperty.propertyID,
//         //                    init.init.LoginUser.userID);
//         //
//         // $vvsApp.socketService
//         //        .socketService
//         //        .subscribe((event: any) => {
//         //            // logger.debug('message received from server... ', event);
//         //        });

//         fixture = TestBed.createComponent(TicketDetailsPage);
//         component = fixture.componentInstance;
//     }));

//     it( "should create component", () => {
//         expect(component).toBeDefined();
//     });


// });
