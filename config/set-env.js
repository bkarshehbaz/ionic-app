...
if (process.env.CI_GIT_REF) {
    const branchName = process.env.CI_GIT_REF.toLowerCase();
    ...
    console.log("Mapped environment key:", envKey);
} else {
    envKey = argv["env"] || argv.env || process.env.env || "prod";
    console.log("No branch name detected, using default env:", envKey);
}
